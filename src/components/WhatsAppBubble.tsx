...
export default function WhatsAppBubble() {
  ...
  useEffect(() => {
    // Double time delay before showing teaser box (8 seconds)
    const timer = setTimeout(() => {
      // Check if user is already past the financing / quotation section before showing
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (targetSection) {
        ...
        // Hide teaser if scroll position passes the quotation/financing elements
        if (rect.bottom <= 0) {
          setShowTeaser(false);
        }
      }
  ...
  return (
    <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-3.5 select-none font-sans">
      ...
      <a
        ...
        className="relative w-20 h-20 bg-[#25D366]/10 rounded-full flex items-center justify-center transition-transform duration-300 transform hover:scale-110 active:scale-95 group relative cursor-pointer"
      ...
      >
        ...
        <div className="relative w-16 h-16 bg-[#25D366] rounded-full flex items-center justify-center shadow-2xl">
          ...
          <svg
            viewBox="0 0 24 24"
            className="w-9 h-9 text-white fill-current animate-bounce"
          >
          ...
          <span className="absolute -top-1 -right-1 w-6 h-6 bg-[#d3080c] text-white font-sans font-black text-xs rounded-full flex items-center justify-center border-2 border-white shadow-md leading-none">
            1
          </span>
        </div>
      </a>
